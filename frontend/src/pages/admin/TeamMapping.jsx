...
const TeamMapping = () => {
    ...
    const totalWorkload = (pods || []).reduce((acc, pod) => acc + (pod.utilization || 0), 0);
    const avgUtilization = totalPods > 0 ? Math.round(totalWorkload / totalPods) : 0;
    ...
    return (
        <DashboardLayout breadcrumbs={['Admin', 'Team Mapping']}>
            <div className="flex items-center justify-between mb-8">
                <div>
                    <h1 className="text-2xl font-bold text-gray-900">Team Mapping & Workflow Control</h1>
            ...
            <div className="flex gap-8 h-[calc(100vh-200px)]">
                ...
                <div className="flex-1 overflow-y-auto custom-scrollbar pr-4 space-y-6">
                    <div className="flex items-center justify-between sticky top-0 bg-[#F8FAFC] py-2 z-10">
                        ...
                        <span className="px-2 py-1 rounded bg-green-100 text-green-600 text-[10px] font-bold">OPTIMIZED</span>
                    </div>

                    <div className="grid grid-cols-1 gap-6">
                        {(pods || []).map((pod) => (
                            <div key={pod.id} className="bg-white border border-gray-100 rounded-2xl p-6 shadow-sm">
                                <div className="flex items-start justify-between mb-6">
                                    <div className="flex items-center gap-4">
                                        <div className="w-14 h-14 rounded-full border-2 border-blue-600 p-0.5">
                                            {pod.photoUrl ? (
                                                <img src={pod.photoUrl} alt="" className="w-full h-full rounded-full object-cover" />
                                            ) : (
                                                <div className="w-full h-full rounded-full bg-blue-100 flex items-center justify-center text-blue-600 font-bold text-xl">
                                                    {pod.firstName?.charAt(0)}
                                                </div>
                                            )}
                                        </div>
                                        <div>
                                            <h4 className="font-bold text-gray-900 text-lg">{pod.firstName} {pod.lastName}</h4>
                                ...
                                <div className="bg-gray-50 rounded-2xl p-4 border border-dashed border-gray-200 min-h-[120px] flex flex-wrap gap-3 items-start">
                                    {(pod.designers || []).map((designer) => (
                                        <div key={designer.id} className="flex items-center gap-3 bg-white px-4 py-3 rounded-xl border border-gray-100 shadow-sm group hover:border-blue-500 transition-all">
                                            ...
                                            <div className="flex flex-col">
                                                <span className="text-xs font-bold text-gray-900">{designer.firstName}</span>
                                    ...
                                    <div className="border-2 border-dashed border-gray-200 rounded-xl h-[52px] w-32 flex items-center justify-center text-gray-400">
                                        <span className="text-[10px] font-medium italic">Drop Here</span>
                                    </div>
                                </div>

                                <div className="mt-4 flex items-center justify-between pt-4 border-t border-gray-50">
                                    <p className="text-xs text-gray-500">Avg. TAT: <span className="font-bold text-gray-900">{pod.avgTAT || '24h'}</span></p>
                ...
                {/* Design Pool Sidebar */}
                <div className="w-80 bg-gray-50 rounded-2xl border border-gray-100 overflow-hidden flex flex-col">
                    <div className="p-6 border-b border-gray-100 bg-white">
                        ...
                        <div className="relative">
                            ...
                            <input
                                ...
                                className="w-full pl-9 pr-4 py-2 bg-gray-100 border-none rounded-xl text-xs focus:ring-2 focus:ring-blue-500/50 transition-all text-gray-900"
                                value={searchQuery}
                    ...
                    <div className="flex-1 overflow-y-auto p-4 space-y-4">
                        {(unassignedDesigners || []).map((designer) => (
                            <div key={designer.id} className="bg-white p-4 rounded-2xl border border-gray-100 shadow-sm hover:border-blue-500 transition-all cursor-grab active:cursor-grabbing group">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div className="flex-1">
                                        <div className="flex items-center justify-between">
                                            <p className="text-sm font-bold text-gray-900 leading-none">{designer.firstName}</p>
                                ...
                                <div className="mt-4 grid grid-cols-2 gap-3">
                                    ...
                                    <div className="bg-gray-50 p-2 rounded-xl">
                                        ...
                                        <p className="text-xs font-bold text-gray-900">0 Active</p>
                                    </div>
                                </div>

                                <div className="mt-4 opacity-0 group-hover:opacity-100 transition-all">
                                    <select
                                        className="w-full bg-blue-50 border-none rounded-lg text-[10px] font-bold text-blue-600 py-1.5"
                                        onChange={(e) => handleAssign(e.target.value, designer.id)}
                                        value=""
                                    >
                                        <option value="" disabled>Assign to Pod...</option>
                                        {(pods || []).map(p => (
                                            <option key={p.id} value={p.id}>{p.firstName}'s Pod</option>
                                        ))}
                                    </select>
                                </div>
                            </div>
